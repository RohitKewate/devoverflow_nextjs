...
import { QuestionShema } from "@/lib/validation"
...
const Question = ({mongoUserId}:Props) => {
    ...
    async function onSubmit(values: z.infer<typeof QuestionShema>) {
        setSubmitting(true)
        try {
            // make a async call to API POST method
            ...
            // Display Errors in the UI for the User
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex w-full flex-col gap-10">
                                    ...
                                    {field.value.length > 0 && (
                                        <div className='flex-start mt-2.5 gap-2.5'>
                                            {field.value.map((tag:any) => (
                                                <Badge key={tag} className='subtle-medium background-light800_dark300 text-light400_light500 flex items-center justify-center gap-2 rounded-md border-none px-4 py-2 capitalize' onClick={() => removeTag(tag,field)}>
                                                    {tag}
                ...
                <Button type="submit" className='primary-gradient w-fit !text-light-900' disabled={submitting}>{submitting ? (
                    <>
